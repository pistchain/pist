...
func (n *Node) OpenDatabase(name string, cache, handles int) (pistdb.Database, error) {
	if n.config.DataDir == "" {
		return pistdb.NewMemDatabase(), nil
	}
	return pistdb.NewLDBDatabase(n.config.ResolvePath(name), cache, handles)
}
