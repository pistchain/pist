...
import (
	...
	flow "git.taiyue.io/pist/go-pist/consensus/tbft/help/flowrate"
...
type MConnection struct {
	...
	quit       chan struct{}
	flushTimer *help.ThrottleTimer // flush writes as necessary but throttled.
	pingTimer  *help.RepeatTimer   // send pings periodically

	// close conn if pong is not received in pongTimeout
	pongTimer     *time.Timer
	pongTimeoutCh chan bool // true - timeout, false - peer sent pong

	chStatsTimer *help.RepeatTimer // update channel stats periodically
...
func (c *MConnection) OnStart() error {
	...
	c.flushTimer = help.NewThrottleTimer("flush", c.config.FlushThrottle)
	c.pingTimer = help.NewRepeatTimer("ping", c.config.PingInterval)
	c.pongTimeoutCh = make(chan bool, 1)
	c.chStatsTimer = help.NewRepeatTimer("chStats", updateStats)
	go c.sendRoutine()
	go c.recvRoutine()
	return nil
}
	...
	for {
		...
		/*
			if numBytes := c.bufConnReader.Buffered(); numBytes > 0 {
				bz, err := c.bufConnReader.Peek(help.MinInt(numBytes, 100))
				if err == nil {
					// return
				} else {
...
func newChannel(conn *MConnection, desc ChannelDescriptor) *Channel {
	...
	if desc.Priority <= 0 {
		help.PanicSanity("Channel default priority must be a positive integer")
	}
	return &Channel{
		conn:                    conn,
		desc:                    desc,
...
func (ch *Channel) nextPacketMsg() PacketMsg {
	...
	packet.Bytes = ch.sending[:help.MinInt(maxSize, len(ch.sending))]
	if len(ch.sending) <= maxSize {
		...
		ch.sending = ch.sending[help.MinInt(maxSize, len(ch.sending)):]
	}
	return packet
}
