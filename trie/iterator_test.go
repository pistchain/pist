...
func TestNodeIteratorCoverage(t *testing.T) {
	...
	for _, key := range db.diskdb.(*pistdb.MemDatabase).Keys() {
...
func TestIteratorNoDpist(t *testing.T) {
	var tr Trie
	for _, val := range testdata1 {
		tr.Update([]byte(val.k), []byte(val.v))
	}
	checkIteratorNoDpist(t, tr.NodeIterator(nil), nil)
}
...
func testIteratorContinueAfterError(t *testing.T, memonly bool) {
	...
	wantNodeCount := checkIteratorNoDpist(t, tr.NodeIterator(nil), nil)
	...
	for i := 0; i < 20; i++ {
		...
		checkIteratorNoDpist(t, it, seen)
...
func checkIteratorNoDpist(t *testing.T, it NodeIterator, seen map[string]bool) int {
	if seen == nil {
		seen = make(map[string]bool)
	}
	for it.Next(true) {
