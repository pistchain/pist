...
import (
	...
	"git.taiyue.io/pist/go-pist/internal/jsre"
	"git.taiyue.io/pist/go-pist/internal/web3ext"
...
func (c *Console) init(preload []string) error {
	...
	flatten := "var pist = web3.pist; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = pist.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines pist.getBalance(pist.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Gpist instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	// Print some generic Gpist metadata
	fmt.Fprintf(c.printer, "Welcome to the Gpist JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("coinbase: " + pist.coinbase);
		console.log("at block: " + pist.blockNumber + " (" + new Date(1000 * pist.getBlock(pist.blockNumber).timestamp) + ")");
