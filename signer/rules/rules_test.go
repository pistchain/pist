...
import (
	...
	"git.taiyue.io/pist/go-pist/signer/core"
	"git.taiyue.io/pist/go-pist/signer/storage"
)
...
func (alwaysDenyUI) OnApprovedTx(tx pistapi.SignTransactionResult) {
	panic("implement me")
}
...
func (d *dummyUI) OnApprovedTx(tx pistapi.SignTransactionResult) {
...
func TestForwarding(t *testing.T) {
	...
	r.OnApprovedTx(pistapi.SignTransactionResult{})
...
func TestLimitWindow(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		response := pistapi.SignTransactionResult{
			Tx:  dummySigned(v),
...
func (d *dontCallMe) OnApprovedTx(tx pistapi.SignTransactionResult) {
	d.t.Fatalf("Did not expect next-handler to be called")
}
