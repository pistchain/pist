...
import (
	...
	"encoding/hex"
	"fmt"
	ethereum "git.taiyue.io/pist/go-pist"
...
func cancel(conn *pistclient.Client, value *big.Int) error {
...
func withdrawImpawn(conn *pistclient.Client, value *big.Int) (common.Hash, error) {
...
func delegateImpawn(conn *pistclient.Client, value *big.Int, address common.Address, key *ecdsa.PrivateKey) (common.Hash, error) {
...
func cancelDImpawn(conn *pistclient.Client, value *big.Int, address common.Address, key *ecdsa.PrivateKey) error {
...
func withdrawDImpawn(conn *pistclient.Client, value *big.Int, address common.Address, key *ecdsa.PrivateKey) (common.Hash, error) {
...
func filterLogs(client *pistclient.Client) {
	method := "Deposit"
	...
	for i := uint64(4706111); i < header.Number.Uint64(); {
		...
		for _, vLog := range logs {
			...
			if method == "SetFee" {
				...
				fmt.Println("value", types.ToPist(deposit.Value), "fee ", deposit.Fee, "pk", hex.EncodeToString(deposit.Pubkey)) // foo
			}
...
func queryRewardImpawn(ctx *cli.Context) error {
	...
	err = client.Call(&head, "pist_getChainRewardContent", "latest", false)
	if err != nil {
		fmt.Println("pist_getChainRewardContent:", err.Error())
		return err
	}
