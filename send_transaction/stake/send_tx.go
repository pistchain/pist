...
func loop(conn *pistclient.Client, header *types.Header, ctx *cli.Context) {
	loopMutex.Lock()
...
func startDelegateTx(conn *pistclient.Client, diff, number uint64, query bool) {
	for i := 0; i < len(delegateAddr); i++ {
		addr := delegateAddr[i]
		key := delegateKey[i]
...
func redistributionDelegate(conn *pistclient.Client) {
	loopCount = 0
...
func deposit(ctx *cli.Context, conn *pistclient.Client, value *big.Int) {
...
func getBalance(conn *pistclient.Client, address common.Address) *big.Int {
...
func sendOtherContractTransaction(client *pistclient.Client, f, toAddress common.Address, value *big.Int, privateKey *ecdsa.PrivateKey, input []byte, method string) (common.Hash, error) {
	blockMutex.Lock()
...
func sendContractTransaction(client *pistclient.Client, from, toAddress common.Address, value *big.Int, privateKey *ecdsa.PrivateKey, input []byte, method string) (common.Hash, error) {
	blockMutex.Lock()
	defer blockMutex.Unlock()
	...
	if types.StakingAddress == toAddress {
		if strings.Contains("withdrawImpawn", method) {
			...
			msg := pistchain.CallMsg{From: from, To: &toAddress, GasPrice: gasPrice, Value: value, Data: input}
...
func getResult(conn *pistclient.Client, txHash common.Hash, contract bool, delegate bool) {
...
func querySendTx(conn *pistclient.Client, number uint64) {
...
func queryTx(conn *pistclient.Client, txHash common.Hash, contract bool, pending bool, delegate bool) bool {
	if pending {
		_, isPending, err := conn.TransactionByHash(context.Background(), txHash)
		if err != nil {
...
func PrintBalance(conn *pistclient.Client, from common.Address) {
	...
	fmt.Println("Your wallet balance is ", trueValue, "'true ", " current Total Stake ", types.ToPist(sbalance))
}
...
func dialConn(ctx *cli.Context) (*pistclient.Client, string) {
	...
	conn, err := pistclient.Dial(url)
	if err != nil {
		log.Fatalf("Failed to connect to the Pistchain client: %v", err)
	}
	return conn, url
}

func getPubKey(ctx *cli.Context, conn *pistclient.Client) (string, []byte, error) {
	var (
		pubkey string
		err    error
	)
...
func printBaseInfo(conn *pistclient.Client, url string) *types.Header {
	header, err := conn.HeaderByNumber(context.Background(), nil)
	if err != nil {
		log.Fatal(err)
	}
...
func queryStakingInfo(conn *pistclient.Client, query bool, delegate bool) (uint64, uint64) {
	header, err := conn.HeaderByNumber(context.Background(), nil)
	if err != nil {
		log.Fatal(err)
	}
	...
	msg := pistchain.CallMsg{From: from, To: &types.StakingAddress, Data: input}
...
func queryDelegateInfo(conn *pistclient.Client, daAddress common.Address) (uint64, uint64) {
	header, err := conn.HeaderByNumber(context.Background(), nil)
	if err != nil {
		log.Fatal(err)
	}
	input := packInput("getDelegate", daAddress, from)
	msg := pistchain.CallMsg{From: daAddress, To: &types.StakingAddress, Data: input}
	output, err := conn.CallContract(context.Background(), msg, header.Number)
	if err != nil {
		printError("method CallContract error", err)
	}
...
func queryRewardInfo(conn *pistclient.Client, snailNumber uint64, address common.Address) (bool, uint64) {
