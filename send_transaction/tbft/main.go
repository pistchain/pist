...
start:
	...
	if err != nil {
		fmt.Println(getTime(), "pist_accounts Error", err.Error())
		return
	}
	if len(account) == 0 {
	...
	// get balance
	var result string = ""
	err = client.Call(&result, "pist_getBalance", account[from], "latest")
	if err != nil {
		fmt.Println(getTime(), "pist_getBalance Error:", err)
		return
	} else {

		bl, _ := new(big.Int).SetString(result, 10)
		fmt.Println(getTime(), "pist_getBalance Ok:", bl, result)
	}
...
func sendTransaction(client *rpc.Client, account []string, wait *sync.WaitGroup) {
	...
	var result string
	client.Call(&result, "pist_sendTransaction", map_data)
	if result != "" {
		Count += 1
	}
}
