...
func send(count int, ip string) {
	...
	err = client.Call(&account, "pist_accounts")
	if err != nil {
		fmt.Println("pist_accounts Error", err.Error())
		msg <- false
		return
	}
	if len(account) == 0 {
...
func sendRawTransaction(client *rpc.Client, from string, to string, value string) (string, error) {
	...
	var result string
	err := client.Call(&result, "pist_sendTransaction", mapData)
	return result, err
}
...
func getAccountBalance(client *rpc.Client, account string) *big.Int {
	...
	// get balance
	err := client.Call(&result, "pist_getBalance", account, "latest")
	if err != nil {
		fmt.Println("pist_getBalance Error:", err)
		msg <- false
		return nil
	}
...
func checkSonAccountBalance(client *rpc.Client, count int, main *big.Int) bool {
	...
	for i, v := range account {
		...
		fmt.Println("pist_getBalance son address ", account[i], " result ", balance, " i ", i, " true ", balanceTrue.Div(balanceTrue, big.NewInt(1000000000000000000)))
