...
// Package p2p implements the Pistchain p2p network protocols.
package p2p
...
import (
	...
	"git.taiyue.io/pist/go-pist/p2p/enr"
	...
	"git.taiyue.io/pist/go-pist/p2p/discover"
...
func (srv *Server) run(dialstate dialer) {
	...
	scheduleTasks := func() {
		...
		if len(runningTasks) < maxActiveDialTasks {
			log.Trace("Server run", "runningTasks", len(runningTasks), "queuedTasks", len(queuedTasks), "peers", len(peers))
...
type NodeInfo struct {
	...
	ENR   string `json:"enr"`   // Pistchain Node Record
