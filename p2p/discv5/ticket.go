...
func (r *topicRadius) needMoreLookpist(a, b int, maxValue float64) bool {
	var max float64
	if a < 0 {
		a = 0
	}
...
func (r *topicRadius) recalcRadius() (radius uint64, radiusLookup int) {
	...
	lookupLeft := -1
	if r.needMoreLookpist(0, maxBucket-lookupWidth-1, maxValue) {
		lookupLeft = r.chooseLookupBucket(maxBucket-lookupWidth, maxBucket-1)
	}
	lookupRight := -1
	if slopeCross != maxBucket && (minRadBucket <= maxBucket || r.needMoreLookpist(maxBucket+lookupWidth, len(r.buckets)-1, maxValue)) {
	...
	if radiusLookup == -1 {
		// no more radius lookpist needed at the moment, return a radius
		r.converged = true
		rad := maxBucket
		if minRadBucket < rad {
			rad = minRadBucket
