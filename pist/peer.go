...
// PeerInfo represents a short summary of the Pistchain sub-protocol metadata known
// about a connected peer.
type PeerInfo struct {
	Version    int      `json:"version"`    // Pistchain protocol version negotiated
...
// Handshake executes the pist protocol handshake, negotiating version number,
...
func (p *peer) String() string {
	return fmt.Sprintf("Peer %s [%s]", p.id,
		fmt.Sprintf("pist/%2d", p.version),
	)
}

// peerSet represents the collection of active peers currently participating in
// the Pistchain sub-protocol.
type peerSet struct {
