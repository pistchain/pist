...
import (
	...
	"git.taiyue.io/pist/go-pist/pist/fetcher"
	"git.taiyue.io/pist/go-pist/pistdb"

	"git.taiyue.io/pist/go-pist/core/forkid"
...
// NewProtocolManager returns a new Pistchain sub protocol manager. The Pistchain sub protocol manages peers capable
// with the Pistchain network.
func NewProtocolManager(config *params.ChainConfig, checkpoint *params.TrustedCheckpoint,
	...
	engine consensus.Engine, blockchain *core.BlockChain, chaindb pistdb.Database,
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Pistchain peer", "peer", id)

	// Unregister the peer from the downloader and Pistchain peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping Pistchain protocol")
	...
	log.Info("Pistchain protocol stopped")
}
...
// handle is the callback invoked to manage the life cycle of an pist peer. When
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Pistchain peer connected", "name", p.Name())

	// Execute the Pistchain handshake
	var (
	...
	if err := p.Handshake(pm.networkID, number, hash, genesis.Hash(), forkid.NewID(pm.blockchain), pm.forkFilter); err != nil {
		p.Log().Debug("Pistchain handshake failed", "err", err)
		return err
	}

	// Register the peer locally
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Pistchain peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Pistchain message handling failed", "err", err)
			return err
		}
	}
}
...
type NodeInfo struct {
	Network    uint64              `json:"network"`    // Pistchain network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
