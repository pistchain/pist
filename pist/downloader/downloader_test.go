...
type downloadTester struct {
	...
	stateDb pistdb.Database // Database used by the tester for syncing from peers
	peerDb  pistdb.Database // Database of the peers containing all data
...
func newTester() *downloadTester {
	...
	tester.stateDb = pistdb.NewMemDatabase()
...
func testEmptyShortCircuit(t *testing.T, protocol int, mode SyncMode) {
	...
	for _, block := range chain.blockm {
		...
		// Actually pistchain has to download block body even there is no transactions, due to exists of pbft signs
...
func testSyncProgress(t *testing.T, protocol int, mode SyncMode) {
	...
	checkProgress(t, tester.downloader, "pristine", pistchain.SyncProgress{})
	...
	<-starting
	checkProgress(t, tester.downloader, "initial", pistchain.SyncProgress{
	...
	<-starting
	checkProgress(t, tester.downloader, "completing", pistchain.SyncProgress{
	...
	progress <- struct{}{}
	pending.Wait()
	checkProgress(t, tester.downloader, "final", pistchain.SyncProgress{
		StartingBlock: uint64(chain.len()/2 - 1),
...
func checkProgress(t *testing.T, d *Downloader, stage string, want pistchain.SyncProgress) {
	// Mark this method as a helper to report errors at callsite, not in here
	t.Helper()
