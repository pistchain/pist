...
import (
	...
	"git.taiyue.io/pist/go-pist/utils"
	...
	"git.taiyue.io/pist/go-pist/crypto/ecies"
...
var (
	oldReceivedMetrics       = metrics.NewRegisteredMeter("pist/pbftAgent/old", nil)
	repeatReceivedMetrics    = metrics.NewRegisteredMeter("pist/pbftAgent/repeat", nil)
	newReceivedMetrics       = metrics.NewRegisteredMeter("pist/pbftAgent/new", nil)
	differentReceivedMetrics = metrics.NewRegisteredMeter("pist/pbftAgent/different", nil)
	nodeHandleMetrics        = metrics.NewRegisteredMeter("pist/pbftAgent/handle", nil)

	tpsMetrics           = metrics.NewRegisteredMeter("pist/pbftAgent/tps", nil)
	pbftConsensusCounter = metrics.NewRegisteredCounter("pist/pbftAgent/pbftConsensus", nil)
)
...
func NewPbftAgent(pist Backend, config *params.ChainConfig, engine consensus.Engine, election *elect.Election, gasFloor, gasCeil uint64) *PbftAgent {
	agent := &PbftAgent{
		config:               config,
		engine:               engine,
		eth:                  pist,
		fastChain:            pist.BlockChain(),
		...
		vmConfig:             vm.Config{EnablePreimageRecording: pist.Config().EnablePreimageRecording},
		gasFloor:             gasFloor,
		gasCeil:              gasCeil,
...
func (agent *PbftAgent) initNodeInfo(pist Backend) {
	...
	if current.Uint64() >= epoch.BeginHeight {
		...
		if block != nil {
			var (
				...
				backpist []*types.CommitteeMember
			)
			for _, m := range agent.fastChain.GetBlockByNumber(epoch.BeginHeight).SwitchInfos() {
				...
				if m.Flag == types.StateUnusedFlag {
					backpist = append(backpist, m)
				}
			}
			committee := &types.ElectionCommittee{Members: members, Backpist: backpist}
