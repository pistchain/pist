...
type Trie interface {
	...
	Prove(key []byte, fromLevel uint, proofDb pistdb.Putter) error
}
...
func NewDatabase(db pistdb.Database) Database {
...
func NewDatabaseWithCache(db pistdb.Database, cache int) Database {
...
func (m cachedTrie) Prove(key []byte, fromLevel uint, proofDb pistdb.Putter) error {
