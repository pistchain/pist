...
import (
	...
	"git.taiyue.io/pist/go-pist/common/prque"
...
type BlockChain struct {
	...
	db     pistdb.Database // Low level persistent database to store final content in
...
// Processor.
func NewBlockChain(db pistdb.Database, cacheConfig *CacheConfig,
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
	...
	for i, block := range blockChain {
		...
		if batch.ValueSize() >= pistdb.IdealBatchSize || len(block.SwitchInfos()) > 0 {
...
func (bc *BlockChain) writeBlockWithState(block *types.Block, receipts []*types.Receipt, state *state.StateDB) (status WriteStatus, err error) {
	...
	if bc.cacheConfig.Disabled {
		...
		if current := block.NumberU64(); current > TriesInMemory {
			...
			if nodes > limit || imgs > 4*1024*1024 {
				triedb.Cap(limit - pistdb.IdealBatchSize)
			}
	...
	// Write the positional metadata for transaction/receipt lookpist and preimages
	rawdb.WriteTxLookupEntries(batch, block)
