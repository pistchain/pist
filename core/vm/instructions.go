...
func opCreate(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error) {
	...
	res, addr, returnGas, suberr := interpreter.evm.Create(callContext.contract, input, gas, bigVal, nil)
...
func opCreate2(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error) {
	...
	res, addr, returnGas, suberr := interpreter.evm.Create2(callContext.contract, input, gas, bigEndowment, &salt, common.Big0)
...
func opCall(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error) {
	...
	ret, returnGas, err := interpreter.evm.Call(callContext.contract, toAddr, args, gas, bigVal, nil)
...
func opSuicide(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error) {
	...
	if interpreter.evm.StateDB.GetPOSLocked(callContext.contract.Address()).Cmp(common.Big0) == 0 {
		interpreter.evm.StateDB.AddBalance(addr, balance)
