...
func sendContractTransaction(client *pistclient.Client, from, toAddress common.Address, value *big.Int, privateKey *ecdsa.PrivateKey, input []byte) common.Hash {
...
func queryTx(conn *pistclient.Client, txHash common.Hash, contract bool, pending bool, delegate bool) {
...
func PrintBalance(conn *pistclient.Client, from common.Address) {
	...
	fmt.Println("Your wallet valid balance is ", trueValue, "'true ", " lock balance is ", types.ToPist(sbalance), "'true ")
}
...
func queryRewardInfo(conn *pistclient.Client, start bool) {
...
func queryStakingInfo(conn *pistclient.Client, query bool, delegate bool) {
	header, err := conn.HeaderByNumber(context.Background(), nil)
	if err != nil {
		log.Fatal(err)
	}
