...
// Package utils contains internal helper functions for pist commands.
package utils
...
import (
	...
	"git.taiyue.io/pist/go-pist/p2p/discv5"

	"git.taiyue.io/pist/go-pist/accounts"
	"git.taiyue.io/pist/go-pist/accounts/keystore"
	"git.taiyue.io/pist/go-pist/common"
	"git.taiyue.io/pist/go-pist/common/fdlimit"
	"git.taiyue.io/pist/go-pist/consensus"

	//"git.taiyue.io/pist/go-pist/consensus/clique"
	"bytes"

	"git.taiyue.io/pist/go-pist/consensus/minerva"
	...
	"git.taiyue.io/pist/go-pist/metrics/influxdb"
	"git.taiyue.io/pist/go-pist/node"
	"git.taiyue.io/pist/go-pist/p2p"
	"git.taiyue.io/pist/go-pist/p2p/enode"
	"git.taiyue.io/pist/go-pist/p2p/nat"
	"git.taiyue.io/pist/go-pist/p2p/netutil"
	...
	"git.taiyue.io/pist/go-pist/piststats"
...
var (
	...
	NetworkIdFlag = cli.Uint64Flag{
		...
		Value: pist.DefaultConfig.NetworkId,
	}
	...
	BFTPortFlag = cli.Uint64Flag{
		...
		Value: uint64(pist.DefaultConfig.Port),
	}
	BFTStandbyPortFlag = cli.Uint64Flag{
		Name:  "bftport2",
		Usage: "committee node standby port ",
		Value: uint64(pist.DefaultConfig.StandbyPort),
	}
	...
	defaultSyncMode = pist.DefaultConfig.SyncMode
	...
	TxPoolPriceLimitFlag = cli.Uint64Flag{
		...
		Value: pist.DefaultConfig.TxPool.PriceLimit,
	}
	TxPoolPriceBumpFlag = cli.Uint64Flag{
		Name:  "txpool.pricebump",
		Usage: "Price bump percentage to replace an already existing transaction",
		Value: pist.DefaultConfig.TxPool.PriceBump,
	}
	TxPoolAccountSlotsFlag = cli.Uint64Flag{
		Name:  "txpool.accountslots",
		Usage: "Minimum number of executable transaction slots guaranteed per account",
		Value: pist.DefaultConfig.TxPool.AccountSlots,
	}
	TxPoolGlobalSlotsFlag = cli.Uint64Flag{
		Name:  "txpool.globalslots",
		Usage: "Maximum number of executable transaction slots for all accounts",
		Value: pist.DefaultConfig.TxPool.GlobalSlots,
	}
	TxPoolAccountQueueFlag = cli.Uint64Flag{
		Name:  "txpool.accountqueue",
		Usage: "Maximum number of non-executable transaction slots permitted per account",
		Value: pist.DefaultConfig.TxPool.AccountQueue,
	}
	TxPoolGlobalQueueFlag = cli.Uint64Flag{
		Name:  "txpool.globalqueue",
		Usage: "Maximum number of non-executable transaction slots for all accounts",
		Value: pist.DefaultConfig.TxPool.GlobalQueue,
	}
	TxPoolLifetimeFlag = cli.DurationFlag{
		Name:  "txpool.lifetime",
		Usage: "Maximum amount of time non-executable transaction are queued",
		Value: pist.DefaultConfig.TxPool.Lifetime,
	}
	...
	GasTargetFlag = cli.Uint64Flag{
		...
		Value: pist.DefaultConfig.MinerGasFloor,
	}

	GasLimitFlag = cli.Uint64Flag{
		Name:  "gaslimit",
		Usage: "Target gas ceiling for fast block",
		Value: pist.DefaultConfig.MinerGasCeil,
	}

	GasPriceFlag = BigFlag{
		Name:  "gasprice",
		Usage: "Minimal gas price to accept for mining a transactions",
		Value: pist.DefaultConfig.GasPrice,
	}
	// Account settings
	...
	PistStatsURLFlag = cli.StringFlag{
		Name:  "piststats",
		Usage: "Reporting URL of a piststats service (nodename:secret@host:port)",
	}
	...
	GpoBlocksFlag = cli.IntFlag{
		...
		Value: pist.DefaultConfig.GPO.Blocks,
	}
	GpoPercentileFlag = cli.IntFlag{
		Name:  "gpopercentile",
		Usage: "Suggested gas price is the given percentile of a set of recent transaction gas prices",
		Value: pist.DefaultConfig.GPO.Percentile,
	}
...
func setBftCommitteeKey(ctx *cli.Context, cfg *pist.Config) {
	var (
...
// for Gpist and returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses!(can search via `gpist account list`)")
	log.Warn("-------------------------------------------------------------------")
...
// SetPistchainConfig applies pist-related command line flags to the config.
func SetPistchainConfig(ctx *cli.Context, stack *node.Node, cfg *pist.Config) {
...
// RegisterPistService adds an Pistchain client to the stack.
func RegisterPistService(stack *node.Node, cfg *pist.Config) {
	var err error
	if cfg.SyncMode == downloader.LightSync {
		Fatalf("Failed to register the Pistchain les service")
	} else {
		err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			fullNode, err := pist.New(ctx, cfg)
			return fullNode, err
		})
	}
	if err != nil {
		Fatalf("Failed to register the Pistchain service: %v", err)
	}
}
...
func RegisterDashboardService(stack *node.Node, cfg *dashboard.Config, commit string) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		// Retrieve both pist services
		var pistServ *pist.Pistchain
		ctx.Service(&pistServ)
		return dashboard.New(cfg, commit, ctx.ResolvePath("logs"), pistServ), nil
	}); err != nil {
		Fatalf("Failed to register the Pistchain Stats service: %v", err)
	}
...
// RegisterPistStatsService configures the Pistchain Stats daemon and adds it to
// th egiven node.
func RegisterPistStatsService(stack *node.Node, url string) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		// Retrieve both pist services
		var pistServ *pist.Pistchain
		ctx.Service(&pistServ)

		return piststats.New(url, pistServ)
	}); err != nil {
		Fatalf("Failed to register the Pistchain Stats service: %v", err)
	}
}
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "gpist.", map[string]string{
				"host": hosttag,
			})
		}
	}
...
func MakeChainDatabase(ctx *cli.Context, stack *node.Node) pistdb.Database {
	var (
...
func MakeChain(ctx *cli.Context, stack *node.Node) (fchain *core.BlockChain, chainDb pistdb.Database) {
	var err error
	...
	cache := &core.CacheConfig{
		...
		TrieNodeLimit: pist.DefaultConfig.TrieCache,
		TrieTimeLimit: pist.DefaultConfig.TrieTimeout,
	}
...
// new format.
//
// e.g. gpist account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// gpist --keystore /tmp/mykeystore --lightkdf account new
//
